...
GPS is no longer a hamburger-menu checkbox — it's a dedicated **My Location button** (`#my-location-btn`, `my_location` Material Symbol), consistent with the Google Maps/Apple Maps convention. It is **not** persisted to localStorage — it always starts OFF on page load (auto-starting a GPS watch without a fresh confirmation would be surprising, and the browser's permission prompt is per-session anyway). `initMyLocationButton()` wires its click handler: toggles `showUserLocation` via `setUserLocationVisible()` and reflects state with an `.active` class (blue icon, `#1967d2`) on success; a geolocation error reverts `showUserLocation` and removes `.active` (see `setUserLocationVisible`'s `watchPosition` error callback).

- **Desktop**: `#my-location-btn` is styled to match Leaflet's default zoom control (white square, `.leaflet-bar`-like border/shadow) and is stacked directly above it. `positionMyLocationButtonDesktop()` measures `.leaflet-control-zoom`'s live `getBoundingClientRect()` and sets `style.bottom` accordingly (not hardcoded — stays correct regardless of the control's actual rendered height), called once after `L.control.zoom().addTo(map)` in `initMap()` and again on `window.resize`.
- **Mobile**: `#my-location-btn` becomes a circular floating button (44px), with a second floating button `#directions-fab-btn` (same style, `directions` icon) stacked directly below it — the **only mobile entry point for Directions now** (the search-bar `#directions-toggle-btn` is `display: none` on mobile; desktop keeps it unchanged). `initFloatingButtonsMobile()` attaches a single `ResizeObserver` to `#sidebar` (the bottom sheet) that recomputes both buttons' `bottom` on every height change — covering drag-driven updates and the sheet's CSS height transitions alike, with no need to hook every individual drag/transition call site. Both buttons' vertical position tracks `min(sheetHeight, 50vh) + gap`, so they rise with the sheet up to the halfway point and then stay pinned there (visually layered above the sheet via `z-index: 1550`, above `.sidebar`'s 1500) rather than being pushed further up or off-screen as the sheet expands toward `full` (90vh) — mirrors Google Maps mobile's locate/directions FAB behavior.
- `toggleDirectionsPanel()` is shared between `#directions-toggle-btn` (desktop + hidden-on-mobile search bar icon) and `#directions-fab-btn` (mobile-only floating button) — both call the same open/close logic, no duplicated state handling.
...
- **Entry points**: `#directions-toggle-btn` (next to `#info-menu-btn` in the search bar, desktop only — hidden on mobile) and `#directions-fab-btn` (mobile-only floating button, below `#my-location-btn`) both call the shared `toggleDirectionsPanel()`, opening the panel with both fields empty. The info overlay's "Dapatkan Arah" button (`.info-overlay-directions-to-here`) calls `dismissOverlay()` then `openDirectionsPanel({ start: {type:'gps', coords:null, ...}, end: {type:'location', coords: location.coords, ...} })` — Start pre-fills to GPS (auto-resolving), End to that location.
...
- **Picking a point**: `renderDirectionsResults(term, field)` pins a "Lokasi Saya (GPS)" row at the top of `#directions-dropdown` (unless excluded, see below), then filters `mapData` (via the shared `matchLocationsByTerm()`, also used by the main search bar) and `mapLabels` — locations with `coords === null` are excluded. Selecting a row calls `setDirectionsField(field, value)`, which updates the input label, drops/moves a colored pin via `renderDirectionsPin()` (`directionsPinPane`, z-index 700 — above `campusMarkerPane`(600) and `userLocationPane`(650)), and triggers `maybeComputeRoute()`.
- **No duplicate endpoints**: `renderDirectionsResults()` reads the *other* field's current value (`directionsStart`/`directionsEnd`) and excludes whatever it currently holds from this field's dropdown — the GPS row is hidden if the other field is `type:'gps'`, a location is excluded by `id` match, a map label is excluded by `coords` reference match (same array reference as `mapLabels[i].coords`, since `setDirectionsField` never clones it). This prevents picking the same point (GPS, location, or label) for both Start and End. Re-evaluated on every dropdown render (focus + input), so it stays correct after swaps/clears.
- **GPS as an endpoint** (`resolveGpsPoint(field)`): if the continuous watch is already running (`currentUserLocation` set via `#my-location-btn`), reuse it immediately. Otherwise call `navigator.geolocation.getCurrentPosition()` **once** — this is deliberately decoupled from `#my-location-btn`/`watchPosition()`: picking GPS in Directions never starts/stops the continuous watch, and toggling the watch never affects an already-resolved directions point (coordinates are a snapshot, not a live reference).
...
```
`CATEGORY_SLUG` maps each `locationType` to a subfolder slug (e.g. `'PENTADBIRAN & PTJ'` → `'pentadbiran'`). The `<img>` starts with `src="${imgBase}.jpg"` and uses chained `onerror` handlers to try `.jpg` → `.png` → `.webp` in order; if all three fail, the image fades to fully transparent (`opacity: 0`). No external placeholder file is required — the placeholder is built from the `material-symbols-outlined` font already loaded on the page.

**Placeholder-first cross-fade (no layout shift):** `.info-overlay-image-wrap` is `position: relative; height: 200px` (fixed) — both `.info-overlay-image` and `.info-overlay-image-placeholder` are `position: absolute; inset: 0`, stacked on top of each other inside that fixed box. The placeholder is visible (`opacity: 1`) by default; the image starts at `opacity: 0`. On the image's `onload`, it fades to `opacity: 1` while the placeholder fades to `opacity: 0` (both via a `0.25s ease` CSS transition declared on each element, triggered by inline `style.opacity` writes in the `onload`/`onerror` handlers). If you ever see a layout "jump" or jerky swap here again, check first whether `.info-overlay-image-wrap`'s fixed `height` still matches between the wrap and what the image/placeholder expect — a height mismatch between the two layers is what caused it the first time (placeholder was `110px`, image was `max-height: 200px`, so swapping caused a visible 90px jump, more noticeable on slower mobile connections where the swap delay is longer).
...
- [ ] `#my-location-btn` is OFF/neutral by default on every page load (not persisted); no hamburger GPS row exists anymore
- [ ] Desktop: `#my-location-btn` renders directly above the Leaflet zoom control, styled to match it; stays correctly stacked above the control after a window resize
- [ ] Mobile: `#my-location-btn` (top) and `#directions-fab-btn` (below it) render as circular floating buttons just above the bottom sheet; the search bar's directions icon (`#directions-toggle-btn`) is hidden
- [ ] Clicking/tapping `#my-location-btn` triggers a browser geolocation permission prompt; accepting shows a blue dot + accuracy circle and turns the button's icon blue (`.active`); tapping again turns GPS off, removes the dot, and reverts the button to neutral
- [ ] Denying the permission prompt reverts `#my-location-btn` to neutral (not stuck in the `.active` state)
- [ ] Dragging the mobile bottom sheet up: both floating buttons rise with the sheet's top edge until the sheet passes 50% screen height, then stay pinned at that 50vh mark (visible above the sheet) as the sheet continues toward `full`
- [ ] Opening a location's info overlay (with coords) always shows both "Buka di Google Maps" (unchanged) and "Dapatkan Arah" — no longer gated on any GPS toggle
...
- [ ] Toggling GPS off via `#my-location-btn` removes the dot/accuracy circle and stops further position updates, but does **not** clear an active Directions route/pins
- [ ] Directions icon (`#directions-toggle-btn` on desktop, `#directions-fab-btn` on mobile) opens the panel with both fields empty; clicking/tapping again closes it
...
- [ ] Picking GPS with the watch OFF shows a one-shot loading state, resolves without turning `#my-location-btn` active; picking GPS with the watch ON resolves instantly
...
- [ ] Picking GPS for Start hides "Lokasi Saya (GPS)" from End's dropdown (and vice versa); picking a specific location/map label for one field hides that same item from the other field's dropdown
