...
// --- LOGIK UNTUK PROGRESSIVE WEB APP (PWA) ---

    // 1. Pendaftaran Service Worker untuk fungsi offline
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
            navigator.serviceWorker.register('/sw.js') // Cari sw.js di root
                .then(registration => {
    ...
    // 2. Logik untuk butang pemasangan PWA
    let deferredPrompt;
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
        // Cegah Chrome 67 dan sebelumnya dari menunjukkan prompt secara automatik
        e.preventDefault();
        // Simpan acara supaya ia boleh dicetuskan kemudian
        deferredPrompt = e;
        // Tunjukkan banner pemasangan kita
        if (installBanner) {
    ...
    if (installButton) {
        installButton.addEventListener('click', (e) => {
            ...
            // Tunjukkan prompt pemasangan rasmi
            if (deferredPrompt) {
                deferredPrompt.prompt();
                // Tunggu pengguna memberi respons kepada prompt
    ...
    // Jika aplikasi sudah dipasang, jangan tunjukkan banner
